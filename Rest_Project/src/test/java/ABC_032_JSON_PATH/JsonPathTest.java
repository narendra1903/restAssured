...
public class JsonPathTest {
   ...
   private final String JSON2 = "[{\"email\":\"name1@mail.com\",\"alias\":\"name one\",\"phone\":\"3456789\"},\n" +
    ...
    public void getFloatAndDoublesAsBigDecimal() {
        final JsonPath using = JsonPath.with(JSON).
   ...
   final Map<String, Object> bicycle = using.<Map<String, Map>>get("store").
		   get("bicycle");
   ...
   using(new JsonPathConfig(JsonPathConfig.NumberReturnType.BIG_DECIMAL)).get("store");
    ...
    public void getNumericalValues() {
        ...
        // The atoms is stored as a long assertThat(JsonPath.with(JSON)
        assertThat(JsonPath.with(JSON).
        		getByte("store.bicycle.atoms"), equalTo((byte) Long.MAX_VALUE));
        assertThat(JsonPath.with(JSON).getShort("store.bicycle.atoms"),
        		equalTo((short) Long.MAX_VALUE));
        assertThat(JsonPath.with(JSON).getInt("store.bicycle.atoms"),
        		equalTo((int) Long.MAX_VALUE));
        assertThat(JsonPath.with(JSON).getLong("store.bicycle.atoms"),
    ...
    public void getMapWithGenericType() {
        final Map<String, String> map = JsonPath.with(JSON_MAP)
    ...
    public void getMapWithAnotherGenericType() {
        final Map<String, Float> map = JsonPath.with(JSON_MAP)
        		.getMap("$", String.class, float.class);
...
/*    @Test
    ...
    public void getObjectAsMapWorksWhenPathPointsToAJsonObject() {
        final Map<String, String> book = JsonPath.from(JSON).
        		getObject("store.book[2]", Map.class);
    ...
    public void getObjectWorksWhenPathPointsToAList() {
        final List<String> categories = JsonPath.from(JSON).
        		getObject("store.book.category", List.class);
    ...
    public void getObjectAsFloatWorksWhenPathPointsToAFloat() {
        final Float price = JsonPath.from(JSON).
        		getObject("store.book.price[0]", Float.class);
    ...
    public void getObjectAsStringWorksWhenPathPointsToAString() {
        final String category = JsonPath.from(JSON).getObject("store.book.category[0]",
        		String.class);
    ...
    parses_json_document_with_attribute_name_equal_to_properties() {
        // Given
  final String jsonWithPropertyAttribute = "[{\"properties\":\"test\"}]";
  // properties is a reserved word in Groovy

        // When
        final String value = new JsonPath(jsonWithPropertyAttribute).
        		getString("[0].properties");
    ...
    unicode_json_values_are_pretty_printed_without_unicode_escaping() {
        final String prettyJson = JsonPath.with("{\"some\":\"ŘÍŠŽŤČÝŮŇÚĚĎÁÉÓ\"}").
        		prettyPrint();
