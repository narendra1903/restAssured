...
public class A008_bodyobj_contains_JsonPath3_Nth_id_details2 
{
...
// Get first record index details	 
	 List<Map<String, Object>> Records =   j1.given(bodyobj).getList("data");  
        ...
        final List<Map<String, ?>> books = JsonPath.with(bodyobj)
                .param("id", author)
                .get("data.id.findAll { data -> data.id == author }");
       
        assertThat(books.size(), equalTo(1));

        final String authorActual = (String) books.get(0).get("email");
